...
public static class SteamworksCloudStoragePatch
{
        ...
        ref bool skipCache)
    {
        try
        {
            ...
            // Validate operation
            if (!ValidateOperation(blobName, "GetBlobsAsync"))
            {
        ...
        ref Il2CppStructArray<byte> data, ref StorageResult __result)
    {
        try
        {
            DebugLogger.Msg($"StandaloneStorage SetBlob: Setting blob '{blobName}' with {data.Length} bytes");
            
            // Force modded blob name
            blobName = ModdedBlobName;
            
            // Validate operation
            if (!ValidateOperation(blobName, "SetBlob"))
            {
    ...
    public static bool EraseAllAsync(ref StorageOperationComplete onComplete)
    {
        try
        {
            ...
            int erasedCount = 0;
            foreach (string filePath in Directory.EnumerateFiles(_storagePath))
            {
                try
                {
    ...
    private static byte[] ProcessDataForModInjection(Il2CppStructArray<byte> data)
    {
        ...
        try
        {
            ...
            JObject obj = JsonConvert.DeserializeObject(json) as JObject ?? new JObject();
            
            // Inject mod data
            obj.Add("ModData", ModOptionsData.ObjectToWrite);
        ...
        {
            throw new InvalidOperationException($"Failed to process data for mod injection: {ex.Message}", ex);
        }
    }
    ...
    private static byte[] ProcessDataForModExtraction(byte[] data)
    {
        ...
        {
            throw new InvalidOperationException($"Failed to process data for mod extraction: {ex.Message}", ex);
        }
    }
    ...
    private static bool ValidateOperation(string blobName, [CallerMemberName] string operation = "")
    {
        if (!_isReady)
        {
